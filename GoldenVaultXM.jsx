...
function AuthModal({ onClose, initialMode = "signup" }) {
  // FIX: Removed `useAuth()` from this component. AuthModal is a pure UI form.
  // All session state (SIGNED_IN / SIGNED_OUT) is handled exclusively by the
  // global onAuthStateChange listener inside AuthProvider, which calls
  // setModal(null) on success. Calling useAuth() here was redundant and, when
  // combined with the orphaned onAuthStateChange inside handleGoogle, created
  // two competing listeners racing to update the same context — the primary
  // trigger of React Error #321 / ghost setState-on-unmounted-component crashes.
  ...
  const handleGoogle = async () => {
    ...
    // FIX: Removed the orphaned supabase.auth.onAuthStateChange subscription
    // that was registered here. It was never unsubscribed on modal close,
    // causing setState calls on an unmounted component after OAuth redirect.
    // AuthProvider's own onAuthStateChange (mounted for the app's lifetime)
    // already handles SIGNED_IN → setUser + setModal(null) + onLogin().
    if (error) { setError(error.message); setGoogleLoading(false); }
    // On success: Supabase redirects the browser. AuthProvider's listener
    // will fire on redirect-back and close the modal automatically.
  };
  ...
  const handle = async () => {
    ...
    // FIX: Removed direct `login(...)` call here. The supabase.auth.signUp /
    // signInWithPassword call triggers a SIGNED_IN event that AuthProvider's
    // global onAuthStateChange listener catches — it then calls setUser,
    // setModal(null), and onLogin(). Calling login() here as well caused a
    // double-update race: both paths fired simultaneously, setting context
    // state twice in the same tick and breaking the Hook call-order invariant.
    setLoading(false);
  };
...
function AuthProvider({ children, onLogin }) {
  ...
  // FIX: Store onLogin in a ref so the effect closure never goes stale.
  // Without this, if the parent re-renders and passes a new onLogin reference,
  // the subscription callback would hold the original (stale) one.
  const onLoginRef = useRef(onLogin);
  useEffect(() => { onLoginRef.current = onLogin; }, [onLogin]);

  // Pick up session on mount (covers Google OAuth redirect-back).
  // FIX: This is the SINGLE source of truth for all auth state changes.
  // AuthModal no longer registers its own competing onAuthStateChange listener.
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        setUser({ name: session.user.user_metadata?.full_name || session.user.email.split("@")[0], email: session.user.email });
        if (onLoginRef.current) onLoginRef.current();
      }
    });
  ...
  }, []); // Empty dep array: runs once on mount, lives for app lifetime

  // FIX: Wrap callbacks in useCallback for stable references across renders,
  // preventing unnecessary re-renders in consumers and broken memoization.
  const login = useCallback((u) => { setUser(u); setModal(null); if (onLoginRef.current) onLoginRef.current(); }, []);
  const logout = useCallback(async () => { await supabase.auth.signOut(); setUser(null); }, []);
  const requireAuth = useCallback((mode = "signup") => { if (!isAuthenticated) { setModal(mode); return false; } return true; }, [isAuthenticated]);
